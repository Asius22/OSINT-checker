...
parser.add_argument('--IP', help='l\'indirizzo IP da analizzare', required=False, default='', type=str)
parser.add_argument('--nv', help='output non verboso, visualiza solo gl ip malevoli', default=False, required=False,  action="store_true")
...
for ip in input:
    ...
    if VT_TOKEN is not None:
        ...
        try:
            is_sus = print_virustotal_response(response.json()['data']['attributes'], print= not verbose)
        except Exception:
            is_sus=False
        if (is_sus):
            MALEVOLI.append(ip)
    ...
    try:
        print_proxycheck_responose(response[ip], is_ioc=is_sus, print=not verbose)
    except Exception:
        None
if len(MALEVOLI):
    print('\n'.join(MALEVOLI))
